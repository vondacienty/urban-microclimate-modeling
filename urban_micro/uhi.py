...
def effect_report(
    ...
    *,
    ...
    """Aggregate scenario details per group and report a compact JSON summary.

    ``details`` follows the ``attribute_effects`` contract: a list of
    ``scenario`` eight-tuples ``(timestamp, cell_id, base, post, delta, cg,
    cr, cm)`` with finite non-boolean int/float numeric fields and unique
    ``(timestamp, cell_id)`` pairs; an empty list yields empty ``groups``.
    ``by`` is ``"time"`` (rows bucketed by Unix epoch, floored to
    ``minutes``-sized buckets) or ``"cell"`` (rows grouped by cell id);
    ``minutes`` must be a non-boolean integer in ``1..1440`` that divides
    1440; ``z`` must be a non-boolean finite number greater than or equal
    to 0.

    Grouping, the six within-group means and the sign-test ``p`` follow
    ``attribute_effects``. Additionally, with ``n`` the group size and
    ``mu`` the delta mean, ``se`` is ``sqrt(sum((delta - mu) ** 2) /
    (n * (n - 1)))`` when ``n > 1`` and 0 otherwise, and
    ``lower = mu - z * se`` / ``upper = mu + z * se``.

    All numbers enter the computation as ``Decimal(str(x))`` under a
    precision-1000, ROUND_HALF_EVEN local context. Returns a compact UTF-8
    JSON string (no spaces, no trailing newline) with top-level keys
    ``by``, ``minutes``, ``z`` and ``groups``; each group object has keys
    ``key``, ``n``, ``base``, ``post``, ``delta``, ``cg``, ``cr``, ``cm``,
    ``p``, ``se``, ``lower`` and ``upper``, sorted by ascending key
    (bucket-start seconds for ``time``, cell id string for ``cell``).
    ``z`` and every numeric result are rendered with exactly 6 decimals,
    ...
    if by not in ("time", "cell"):
        raise ValueError("by must be 'time' or 'cell'")
    minutes = _validate_minutes(minutes)
    z_d = _validate_grid_number(z, "z", positive=False)
