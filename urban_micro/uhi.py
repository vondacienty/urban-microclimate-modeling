...
def frontier_joint_intervals(
    ...
    *,
    ...
    """Merge adjacent joint-frontier limits sharing a modal pick.

    ``reports``, ``weights``, ``cost``, ``limits``, ``cases`` and
    ``alpha`` follow the :func:`frontier_joint` contract exactly — the
    same type and value validation applies, each candidate's ``score``
    and ``significant`` are computed exactly as there and each case's
    winning subset at every limit is the same exhaustive-search winner
    (maximum total gain, then minimum total cost, then the
    lexicographically ascending selected-key list).

    Limits are processed in ascending order; at each limit the modal
    pick is the most frequent winning pick across the cases with ties
    broken by the lexicographically ascending selected-key list.
    Maximal runs of adjacent limits sharing the same modal pick are
    merged into intervals. For each interval ``start`` and ``end`` are
    the first and last limit of the run, ``threshold`` is ``null`` for
    the first interval and the interval's ``start`` otherwise, ``n`` is
    the number of limits in the run and ``stability`` is the minimum
    across the run's limits of the modal pick's frequency divided by
    the number of cases. For every case ``cost`` and ``gain`` are the
    means of its winning subset's total effective cost and total
    effective gain across the run's limits and ``support`` is the share
    of the run's limits where the case's winning pick equals the modal
    pick; ``worst`` is the key of the case with the smallest ``gain``,
    ties broken by the smaller key. All arithmetic is
    ``Decimal(str(x))`` under a precision-1000, ROUND_HALF_EVEN context
    and comparisons use the unquantized values.
    ...
    string array, ``n`` is an integer, ``worst`` and ``key`` are
    strings, ``threshold`` is ``null`` or a number and every numeric
    ...
    with localcontext() as ctx:
        ...
        # per-limit modal pick and its frequency across the cases
        point_modals: list[tuple[tuple[str, ...], int]] = []
        for limit_index in range(len(limit_values)):
            ...
            point_modals.append((modal_pick, modal_frequency))

        # maximal runs of adjacent limits sharing the modal pick, as
        # [first, last) index pairs into limit_values
        runs: list[tuple[int, int]] = []
        run_start = 0
        for limit_index in range(1, len(limit_values)):
            if point_modals[limit_index][0] != point_modals[run_start][0]:
                runs.append((run_start, limit_index))
                run_start = limit_index
        runs.append((run_start, len(limit_values)))

        interval_strings: list[str] = []
        for run_index, (first, last) in enumerate(runs):
            n = last - first
            modal_pick = point_modals[first][0]
            stability = min(
                Decimal(point_modals[i][1]) / Decimal(case_total)
            ...
            for case_key in case_keys:
                mean_cost = sum(
                    (case_winners[case_key][i][1] for i in range(first, last)),
                    Decimal(0),
                ) / Decimal(n)
                mean_gain = sum(
                    (case_winners[case_key][i][0] for i in range(first, last)),
                    Decimal(0),
                ) / Decimal(n)
                support_count = sum(
                    1
            ...
            interval_strings.append(
                ...
                + ',"end":' + _format6(limit_values[last - 1])
                + ',"threshold":'
                + (
                    "null"
                    if run_index == 0
                    else _format6(limit_values[first])
                )
                + ',"pick":'
                + json.dumps(
                    list(modal_pick),
