...
def panel_priority_frontier(
    ...
    *,
    ...
    """Trace the optimal significant-candidate subset across budget limits.

    ``reports``, ``weights``, ``cost`` and ``alpha`` follow the
    :func:`panel_priority` contract. ``limits`` must be a non-empty
    list of non-negative finite non-boolean int/float values whose
    ``Decimal(str(x))`` values are pairwise distinct. A non-dict
    ``reports``, ``weights`` or ``cost`` or a non-list ``limits``
    raises ``TypeError``; every other contract violation raises
    ``ValueError``.

    Each candidate's ``score`` and ``significant`` flag are computed
    exactly as in :func:`panel_priority`. The limits are sorted
    ascending and, for each limit ``L``, every subset of the
    significant candidates whose total cost is at most ``L`` is
    enumerated exhaustively — the empty subset is always feasible —
    and the winner maximizes total score, then minimizes total cost,
    then minimizes the lexicographically ascending selected-key list.
    The first entry's previous score is 0 and its previous pick is
    empty; each later entry uses the previous entry's result.
    ``marginal`` is the entry's total score minus the previous total
    score and ``added``/``removed`` are the ascending set differences
    between the entry's pick and the previous pick. All arithmetic is
    ``Decimal(str(x))`` under a precision-1000, ROUND_HALF_EVEN
    context and comparisons use the unquantized values.

    Returns a compact UTF-8 JSON string with no spaces and exactly one
    trailing newline; the top-level key order is ``alpha, frontier``.
    Each frontier entry uses the key order ``limit, cost, score,
    marginal, pick, added, removed`` and entries appear in ascending
    limit order. ``pick``, ``added`` and ``removed`` are arrays of
    strings and every numeric value renders with six decimals,
    ...
    for limit in limits:
        ...
        if limit_value in limit_values:
