...
def surface_thermal_attribution_report(items: list, actions: list) -> str:
    """Attribute the thermal-load effect of each action to its cell and kind.

    ``items`` follows the same ``(cell_id, kind, height, area, grid_area,
    albedo, emissivity)`` seven-tuple contract as
    :func:`surface_thermal_report`. ``actions`` follows the same strict
    ``(cell_id, kind, d_albedo, d_emissivity)`` four-tuple contract as
    :func:`surface_thermal_scenario_report`: ``cell_id`` must occur in
    ``items``, ``kind`` must be one of ``roof``, ``impervious``,
    ``green`` or ``other`` and the cell must contain a coverage item of
    that kind, ``d_albedo``/``d_emissivity`` must be non-boolean finite
    int/float, each ``(cell_id, kind)`` pair may appear at most once and
    after applying the delta every affected item must still satisfy
    ``albedo`` in ``[0, 1]`` and ``emissivity`` in ``(0, 1]``.

    Each action acts on every coverage item of the named kind in the
    named cell. Its entry carries the area fraction
    ``fraction = sum(a)/grid_area`` of that kind in the cell, the base
    load share ``base = sum(a*(1-A)*E)/grid_area`` and the same
    expression after applying the action ``post``, plus
    ``delta = post - base``.

    All arithmetic uses ``Decimal(str(x))`` under a precision-1000,
    ROUND_HALF_EVEN local context. Entries are emitted in ascending cell
    id order, with kinds ordered roof, impervious, green, other. Returns
    a compact JSON string with no spaces and no trailing newline; the
    top-level key is ``actions`` and each object uses the key order
