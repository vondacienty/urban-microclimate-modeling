...
def effect_matrix_robust_report(
    ...
    *,
    ...
    """Aggregate scenario deltas into a time-bucket x cell robust (MAD) report.
    ...
    ``floor(t / (minutes * 60)) * (minutes * 60)``; buckets are emitted in
    ascending order and, within each bucket, cells in ascending string order.
    Within each cell the deltas ``d`` are sorted ascending. The order
    statistic is ``Q(q)`` with ``r = (n - 1) * q``, ``i = floor(r)``,
    ``f = r - i`` and
    ``Q = (1 - f) * d[i] + f * d[min(i + 1, n - 1)]``; ``median`` is
    ``Q(0.5)``. ``mad`` is ``Q(0.5)`` of the sorted absolute deviations
    ``|d - median|`` and ``se = 1.4826 * mad / sqrt(n)``;
    ``lower``/``upper`` are ``median - z * se`` / ``median + z * se``.
    ...
    empty ``details`` yields ``{"minutes":60,"z":1.960000,"groups":[]}``.
    ``z`` and every numeric result are rendered with exactly six decimals,
    negative zero normalized to ``0.000000``; bucket keys and cell sizes are
    integers and cell ids are JSON-escaped with Unicode preserved.
    ``details`` not being a list raises ``TypeError``; every other contract
    violation raises ``ValueError``.
    """
    if not isinstance(details, list):
    ...
    with localcontext() as ctx:
        ...
        def quantile(sorted_values: list[Decimal], q: Decimal) -> Decimal:
            ...
            f = r - Decimal(i)
            j = min(i + 1, n - 1)
            return (Decimal(1) - f) * sorted_values[i] + f * sorted_values[j]
