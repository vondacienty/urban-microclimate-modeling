...
def energy_temperature_scenario_uhi_report(
    ...
    """Join scenario-adjusted energy residuals and temperatures into a
    per-zone UHI report.

    ``energy`` is a list of strict six-tuples ``(timestamp, cell_id,
    net_rad, sensible, latent, storage)``, ``temp`` a list of strict
    three-tuples ``(timestamp, cell_id, value)`` and ``actions`` a list
    of strict five-tuples ``(cell_id, dn, ds, dl, dst)``: ``timestamp``
    must be a non-boolean non-negative integer, ``cell_id`` a non-empty
    string and the numeric fields finite non-boolean int/float values;
    ``(timestamp, cell_id)`` pairs must be unique within each table,
    each action ``cell_id`` must occur in ``energy`` and each cell may
    carry at most one action. ``zones`` maps non-empty cell id strings
    to ``'urban'`` or ``'rural'`` and must cover every cell id occurring
    in ``energy`` and ``temp``. ``minutes`` must be a non-boolean integer
    in ``1..1440`` that divides 1440.
    ...
    present in both inputs are kept. For each kept pair the action
    increments give ``a = dn - ds - dl - dst`` (zero when the cell has no
    action) and ``post = r + a``. The per-cell ``r``, ``post`` and ``v``
    are then averaged per zone within each bucket; a bucket is dropped
    when either zone has no contributing cell. The three reported
    differences are the urban mean minus the rural mean, in order, of
    ``r`` (``base``), ``post`` and ``v`` (``uhi``).
    ...
    for _, cell_id, *_ in parsed_energy:
        if cell_id not in zones:
            raise ValueError(f"zones is missing cell: {cell_id!r}")
    for _, cell_id, _ in parsed_temp:
        if cell_id not in zones:
    ...
    with localcontext() as ctx:
        ...
        # bucket -> zone -> [r sum, post sum, v sum, cell count]
        zone_acc: dict[int, dict[str, list]] = {}
        for key, (residual_total, energy_count) in energy_acc.items():
            temp_entry = temp_acc.get(key)
            if temp_entry is None:
            ...
            temp_total, temp_count = temp_entry
            bucket, cell_id = key
            residual_mean = residual_total / energy_count
            temp_mean = temp_total / temp_count
            dn, ds, dl, dst = action_map.get(
                cell_id, (zero, zero, zero, zero)
            )
            post_mean = residual_mean + dn - ds - dl - dst
            acc = zone_acc.setdefault(bucket, {}).setdefault(
        ...
        for bucket in sorted(zone_acc):
            ...
            if not urban or not rural:
                continue
            base = urban[0] / urban[3] - rural[0] / rural[3]
            post = urban[1] / urban[3] - rural[1] / rural[3]
            uhi = urban[2] / urban[3] - rural[2] / rural[3]
            groups.append(
                '{"key":' + str(bucket)
                + ',"base":"' + _format6(base) + '"'
                + ',"post":"' + _format6(post) + '"'
