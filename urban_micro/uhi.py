...
def effect_matrix_hotspot_report(
    ...
    *,
    ...
    """Aggregate scenario deltas per bucket and emit a local-Moran hotspot JSON report.
    ...
    With ``N`` the number of cells in the bucket, the p-values are ranked
    ascending by ``(p, cell)`` and each rank ``j`` (1-based) gets the
    Benjamini-Hochberg q-value
    ``q_j = min(1, min(N * p_l / l for l in j..N))``, mapped back to its
    cell; ``reject`` is ``q <= alpha``, compared on the unquantized values.
    With ``v_c`` the neighbor sum ``sum(x_j over the neighbors of c)``, a
    rejected cell with both ``x_c`` and ``v_c`` non-zero is classified by
    the sign pair ``(sign(x_c), sign(v_c))``: ``(+, +)`` -> ``"HH"``,
    ``(-, -)`` -> ``"LL"``, ``(+, -)`` -> ``"HL"`` and ``(-, +)`` ->
    ``"LH"``; every non-rejected cell (and any rejected cell with a zero
    ``x_c`` or ``v_c``) is ``"NS"``.
    ...
    ascending cell id order. ``local``, ``p``, ``q`` and ``alpha`` are
        ...
        for bucket in sorted(buckets):
            ...
            if n > _HOTSPOT_MAX_N:
                raise ValueError(
                    ...
                    f"requires at most {_HOTSPOT_MAX_N} cells per bucket"
                )
            d = []
            for cell_id in cells:
                total, count = buckets[bucket][cell_id]
            ...
            deviations = [value - mean_d for value in d]
            sxx = sum((value * value for value in deviations), Fraction(0))
            # Spatial lag of each cell: sum of neighbor deviations (0 when
            # the cell has no in-bucket neighbor).
            lags = [
            ...
            if sxx == 0:
                ...
                observed = [
                    Fraction(n) * deviations[index] * lags[index] / sxx
                    for index in range(n)
                ]
                active = [bool(neighbors_of) for neighbors_of in adjacency]
                # Write d_i = p_i / D with a shared denominator D and put
            ...
            # Benjamini-Hochberg q-values within this bucket: rank ascending
            # by (p, cell), then accumulate the running minimum of
            # N * p_l / l from the top rank down, mapping q back to each cell.
            order = sorted(
                range(n), key=lambda index: (p_values[index], index)
            )
            q_values: list[Fraction] = [Fraction(1)] * n
            running = Fraction(1)
            for rank in range(n, 0, -1):
                index = order[rank - 1]
                candidate = Fraction(n) * p_values[index] / rank
                if candidate < running:
                    running = candidate
            ...
            cell_items = []
            for index, cell_id in enumerate(cells):
                reject = q_values[index] <= alpha_fraction
                xc = deviations[index]
                vc = lags[index]
                ...
                cell_items.append(
                    ...
                    + ',"local":' + _format6(_fraction_to_decimal(observed[index]))
                    + ',"p":' + _format6(_fraction_to_decimal(p_values[index]))
                    + ',"q":' + _format6(_fraction_to_decimal(q_values[index]))
                    + ',"reject":' + ("true" if reject else "false")
