...
def _validate_stations(stations: object) -> dict:
    ...
    for station_id, zone in stations.items():
        if zone not in _ZONES:
            raise ValueError(
                f"station {station_id} must map to 'urban' or 'rural', got {zone!r}"
            )
    return stations
...
def _validate_cells(cells: object) -> dict:
    ...
    for station_id, cell_id in cells.items():
        if not isinstance(cell_id, str) or not cell_id:
            raise ValueError(
                f"station {station_id} must map to a non-empty grid cell ID "
...
def _validate_record(record: object, stations: dict) -> tuple[object, int, Decimal]:
    ...
    if set(record.keys()) != _RECORD_KEYS:
        raise ValueError(
            ...
            "'station_id', 'temp_c' and 'timestamp'"
        )
...
def _precision_for(temps: list[Decimal]) -> int:
    ...
    least significant digit, plus carry digits from adding ``len(temps)`` values),
    the 3 quantization decimals and guard digits for division.
    """
    max_adj = 0
    min_exp = 0
    count = 0
    for temp in temps:
        ...
        if not temp.is_zero():
            ...
            min_exp = min(temp.as_tuple().exponent, min_exp)
    span = max_adj - min_exp + 1
    return span + len(str(count)) + 10
...
def compute_uhi(
    ...
    *,
    ...
    local decimal context whose precision is raised to fit the digit span of
...
def align_temp(
    ...
    *,
    ...
    ``station`` records follow the ``compute_uhi`` three-key record contract
