...
def effect_matrix_trimmed_report(
    ...
    *,
    ...
    ``floor(t / (minutes * 60)) * (minutes * 60)``; buckets are emitted in
    ascending order and, within each bucket, cells in ascending string
    order. Within each bucket/cell the deltas are sorted ascending; with
    ``n`` the cell size and ``k = floor(trim * n)``, the trimmed sample is
    ``d[k:n - k]`` with ``m = n - 2 * k`` values; ``m < 2`` raises
    ``ValueError``. ``trimmed`` is the trimmed mean ``mu = sum(d) / m``;
    ``p`` is the exact two-sided sign-flip permutation p-value of the
    trimmed sample: all ``2 ** m`` sign vectors ``s_i`` in ``{-1, 1}`` are
    enumerated and ``p = 2 ** -m * #{|sum(s_i * d_i) / m| >= |mu|}``. The
    standard error is ``sqrt(sum((d_i - mu) ** 2) / (m * (m - 1)))`` and
    ``lower``/``upper`` are ``mu - z * se`` / ``mu + z * se``.
    ...
    ``z`` and every numeric result are rendered with exactly six decimals,
    negative zero normalized to ``0.000000``; bucket keys and cell sizes
    are integers and cell ids are JSON-escaped with Unicode preserved.
    ``details`` not being a list raises ``TypeError``; every other contract
    violation raises ``ValueError``.
    """
    if not isinstance(details, list):
    ...
    with localcontext() as ctx:
        ...
        for bucket in sorted(buckets):
            ...
            for cell_id in sorted(buckets[bucket]):
                deltas = sorted(buckets[bucket][cell_id])
                n = len(deltas)
                k = int((trim_value * n).to_integral_value(rounding=ROUND_FLOOR))
                trimmed = deltas[k:n - k]
                m = len(trimmed)
                if m < 2:
                    raise ValueError(
                        f"bucket {bucket} cell {cell_id!r} keeps {m} of {n} "
                        "rows after trimming; trimmed report requires at "
                        "least 2 rows per bucket/cell"
                    )
                total = Decimal(0)
