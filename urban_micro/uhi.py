...
def effect_fdr_report(
    ...
    *,
    ...
    """Aggregate scenario deltas per group and emit an FDR-corrected JSON report.
    ...
    two-sided sign-test p-value of ``attribute_effects``: with ``r``/``s``
    the counts of positive/negative deltas (zeros ignored), ``m = r + s``
    and ``q0 = min(r, s)``, ``p`` is 1 when ``m`` is 0 and
    ``min(1, 2 * sum(C(m, k) for k in 0..q0) / 2**m)`` otherwise.

    The Benjamini-Hochberg correction is applied across the ``N`` groups:
    sorted by ascending ``(p, key)`` with rank ``j`` (1-based), each group
    gets ``q_j = min(1, min(N * p_l / l for l in j..N))`` mapped back to its
    key, and ``reject`` is ``q <= alpha``.
    ...
    with localcontext() as ctx:
        ...
        # group key -> (n, mean delta, sign-test p)
        ...
        # Benjamini-Hochberg q-values: sweep ranks from largest to smallest,
        # keeping the running minimum of N * p_l / l.
        ...
        for rank in range(group_count, 0, -1):
            group_key = ranked[rank - 1]
            candidate = Decimal(group_count) * stats[group_key][2] / rank
            if running_min is None or candidate < running_min:
                running_min = candidate
            q_values[group_key] = (
                running_min if running_min < 1 else Decimal(1)
            )

        items = []
        for group_key in sorted(groups):
            n, mu, p = stats[group_key]
            q = q_values[group_key]
            if by == "time":
                key_json = str(group_key)
            else:
                key_json = json.dumps(group_key, ensure_ascii=False)
            items.append(
                ...
                + ',"p":' + _format6(p)
                + ',"q":' + _format6(q)
                + ',"reject":' + ("true" if q <= alpha_value else "false")
                + '}'
            )
