...
def energy_temperature_report(energy: list, temp: list, *, minutes: int = 60) -> str:
    """Join energy-balance residuals with temperatures per bucket and cell.

    ``energy`` is a list of six-tuples ``(timestamp, cell_id, net_rad,
    sensible, latent, storage)`` and ``temp`` a list of three-tuples
    ``(timestamp, cell_id, v)``: ``timestamp`` must be a non-boolean
    non-negative integer, ``cell_id`` a non-empty string and every numeric
    field a finite non-boolean int/float; ``(timestamp, cell_id)`` pairs
    must be unique within each table. ``minutes`` must be a non-boolean
    integer in ``1..1440`` that divides 1440.

    Both tables are bucketed by Unix epoch with key
    ``floor(t / (minutes * 60)) * (minutes * 60)``. Within each
    ``(bucket, cell_id)`` pair the per-row energy residuals
    ``r = net_rad - sensible - latent - storage`` and the ``temp`` values
    are averaged separately, and only pairs present in both tables are
    kept. Each kept cell object carries the two group sizes ``n_energy``
    and ``n_temp``, the mean ``residual``, the mean ``temp`` and
    ...
    JSON string with no spaces and no trailing newline; the top-level key
    order is ``minutes, groups``, each group object uses the key order
    ``key, cells`` and each cell object uses the key order ``key,
    n_energy, n_temp, residual, temp, coupling`` with ``key`` the cell id.
    Buckets are emitted in ascending order and cells within each bucket in
    ascending string order. Bucket keys and the counts are integers and
    every other numeric result is rendered as a string with exactly six
    ...
    seen = set()
    for row in temp:
        validated = _validate_airflow_row(row)
        key = (validated[0], validated[1])
        if key in seen:
            raise ValueError(f"duplicate (timestamp, cell_id) pair: {key!r}")
        seen.add(key)
    ...
    with localcontext() as ctx:
        ...
        bucket_seconds = minutes * 60
        # (bucket, cell_id) -> [residual sum, count]
        energy_acc: dict[tuple[int, str], list] = {}
        for timestamp, cell_id, net_rad, sensible, latent, storage in parsed_energy:
            bucket = (timestamp // bucket_seconds) * bucket_seconds
            acc = energy_acc.setdefault((bucket, cell_id), [Decimal(0), 0])
            acc[0] += net_rad - sensible - latent - storage
            acc[1] += 1

        # (bucket, cell_id) -> [temp sum, count]
